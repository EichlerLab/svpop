...
rule tracks_sv_bb:
    input:
        bed='temp/tracks/variant/{sourcetype}/{sourcename}/{sample}/{svset}/{filter}/{vartype}_{svtype}.bed',
        asfile='temp/tracks/variant/{sourcetype}/{sourcename}/{sample}/{svset}/{filter}/{vartype}_{svtype}.as',
        fai=config['reference_fai']
    output:
        bb='tracks/variant/{sourcetype}/{sourcename}/{sample}/{svset}/{filter}/{vartype}_{svtype}.bb'
    shell:
...
rule tracks_sv_bed:
    input:
        ...
        fai=config['reference_fai']
    output:
        bed=temp('temp/tracks/variant/{sourcetype}/{sourcename}/{sample}/{svset}/{filter}/{vartype}_{svtype}.bed'),
        asfile=temp('temp/tracks/variant/{sourcetype}/{sourcename}/{sample}/{svset}/{filter}/{vartype}_{svtype}.as')
    run:
        ...
        # Make track
        analib.tracks.variant.make_bb_track(df, df_fai, output.bed, output.asfile, track_name, track_description)
