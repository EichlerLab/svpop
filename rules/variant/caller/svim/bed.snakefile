"""
Process svim variant calls.
"""

# variant_svim_bed
#
# Write filtered BED and empty FASTA files.
rule variant_svim_bed:
    input:
        bed='temp/variant/caller/svim-{seq_set}/bed/{sample}/all/{filter}/byref/{vartype}_{svtype}.bed'
    output:
        bed='results/variant/caller/svim-{seq_set}/bed/{sample}/all/{filter}/byref/{vartype}_{svtype}.bed',
        fa='results/variant/caller/svim-{seq_set}/fasta/{sample}/all/{filter}/{vartype}_{svtype}.fa.gz'
...
# variant_svim_bed_filter_region
#
# Apply a BED filter to SVs.
rule variant_svim_bed_filter_region:
    input:
        bed='temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/{vartype}_{svtype}.bed',
        filter=lambda wildcards: analib.variant.get_filter_bed(wildcards.filter, UCSC_REF_NAME, config, SVPOP_DIR)
    output:
        bed=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/{filter}/byref/{vartype}_{svtype}.bed'),
        bed_filt='results/variant/caller/svim-{seq_set}/bed/{sample}/all/{filter}/byref/removed/{vartype}_{svtype}_filtered.bed'
    wildcard_constraints:
        filter='((?!all)[^\/]*)|all.+',
        svtype='ins|del|inv|dup'
    run:
...
#
# TSV to BED.
rule variant_svim_tsv_to_bed:
    input:
        tsv='temp/variant/caller/svim-{seq_set}/bed/{sample}/tsv/variants.tsv.gz'
    output:
        sv_ins=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/sv_ins.bed'),
        sv_del=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/sv_del.bed'),
        sv_inv=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/sv_inv.bed'),
        indel_del=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/indel_del.bed'),
        indel_ins=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/all/all/indel_ins.bed'),
        tsv_filt='results/variant/caller/svim-{seq_set}/bed/{sample}/all/all/filtered/vcf_fail.tsv.gz'
    run:
...
#
# VCF to TSV.
rule variant_svim_vcf_to_tsv:
    input:
        vcf=lambda wildcards: variant_global_sample_table_entry('svim', wildcards=wildcards)['DATA']
    output:
        tsv=temp('temp/variant/caller/svim-{seq_set}/bed/{sample}/tsv/variants.tsv.gz')
    shell:
        """
        bcftools query -H -f"%CHROM\\t%POS\\t%ID\\t%REF\\t%ALT\\t%QUAL\\t%FILTER\\t%INFO/SVTYPE\\t%INFO/END\\t%INFO/SVLEN\\t%INFO/SUPPORT\\t%INFO/STD_SPAN\\t%INFO/STD_POS\t%INFO/READS[\\t%GT\\t%CN\\t%DP\\t%AD]\n" {input.vcf} | """
        """gzip """
        """> {output.tsv}"""
