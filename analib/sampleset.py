...
def get_sample_set_input(sampleset_name, sampleset_list, file_pattern, config, wildcards=None, as_tuple=False):
    ...
    analib.sm.nlset(wildcards, 'sourcetype', sampleset_entry['sourcetype'])
    analib.sm.nlset(wildcards, 'sourcename', sampleset_entry['sourcename'])

    for sample_name in sampleset_entry['samples']:
        analib.sm.nlset(wildcards, 'sample', sample_name)
