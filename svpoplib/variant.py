...
def vcf_tsv_to_bed(
        tsv_in, sample=None, bed_file=None, filt_file=None, chunk_size=6000, threads=1, callback_pre_bed=None,
    ...
    record pass, then the iterator returns nothing.
    ...
    :param filter_gt: Filter on the GT column if `True` or `None`. If there is no GT column and the parameter is `True`,
        an error is thrown. If ther is no GT column and the parameter is `None`, then no filtering occurs.
    ...
    REQUIRED_COLS = {'CHROM', 'POS', 'REF', 'ALT', 'QUAL', 'FILTER'}
    ...
    for df in df_iter:
        ...
        if col_names is None:
            ...
            if sample is not None:
                ...
                if len(samples) > 1 and sample not in samples:
                    raise RuntimeError(
            ...
            if compute_af is None:
                compute_af = len(set(col_names) & {'AC', 'AN', 'AF'}) == 0
        ...
        if len(gt_cols) > 0:
            gt = df[gt_cols].fillna('.').apply(lambda row: '/'.join(row.astype(str)), axis=1)

            gt_alt_set = gt.apply(lambda val: set(re.split('[|/]', val)))

            # TODO - CCONTINUE HERE

            df = df.loc[
                [str(alt_idx) in alt_set for alt_idx, alt_set in zip(df['VCF_ALT_IDX'], gt_alt_set)]
            ]
