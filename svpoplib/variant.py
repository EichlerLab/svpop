...
def vcf_tsv_to_bed(tsv_in, bed_file=None, filt_file=None, chunk_size=6000, threads=1, callback_pre_bed=None):
    """
    ...
    for df in df_iter:
        ...
        if col_names is None:
            ...
            if len(samples) > 1:
                ...
                col_names = [col for col in df.columns if
                             not re.match('.+:.+', col) or col.startswith(wildcards.sample + ':')]
        ...
        if callback_pre_bed is not None:
            ...
            if df_reformat is None:
                ...
                df_filt = df.loc[[val for val in df.index if val not in id_pass_set]]
