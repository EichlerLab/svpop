...
def vcf_fields_to_seq(row, pos_row='POS', ref_row='REF', alt_row='ALT'):
    ...
    if alt[0] == '<' and alt[-1] == '>':
        ...
        if svtype in {'INS', 'DEL', 'INV', 'DUP', 'CNV'}:
            # Get length
            svlen = None
...
def vcf_tsv_to_bed(
    ...
    This function can process data in batches if `batch` and `batch_count` is set. Each chunk of the input DataFrame
    has an index (first chunk has index 0, second has index 1, etc). If batching is set, then only chunks matching
    the chunk index are processed. WARNING: This requires that `chunk_size` and `batch_count` are the same across
    all batches and that this function is called with all batches from 0 to `batch_count - 1` and results concatenated.
    Failing these invariants will cause duplicate or dropped records.
    ...
    :param ref_fa_name: Path to the reference file. Used for retrieving missing reference alleles. Only required if
        there are missing reference alleles for non-symbolic alternate variants (variant in REF and ALT, INFO fields).
    :param batch: The batch number if processing in batches, otherwise None.
    :param batch_count: The number of batches in processing in batches, otherwise None.
    ...
    if batch is not None or batch_count is not None:

        if batch is None or batch_count is None:
            raise RuntimeError(f'Must set both "batch" ({batch}) and "batch_count" ({batch_count}')

        if batch < 0:
            raise RuntimeError(f'Argument "batch" is negative: {batch}')

        if batch_count < 2:
            raise RuntimeError(f'Argument "batch_count" must be at least 2 (set "batch" and "batch_count" to None to process all records at once): {batch_count}')

        if batch >= batch_count:
            raise RuntimeError(f'Argument "batch" ({batch}) must be less than "batch_count" ({batch_count})')
    ...
    # Facilities for resolving missing reference sequences
    if ref_fa_name is not None:
        ref_fa = pysam.FastaFile(ref_fa_name)
        missing_ref_pattern = re.compile(f'^[nN]+$')
    else:
        ref_fa = None
        missing_ref_pattern = None

    chrom_cache_name = None
    chrom_cache_seq = None
    ...
    df = None
    df_iter_index = -1  # Index of the df_iter iteration (first iteration is 0, then 1, etc).
    ...
    for df in df_iter:
        ...
        # Check batch
        if batch is not None and df_iter_index % batch_count != batch:
            continue
        ...
        # Fill missing REF column
        if ref_fa is not None:
            df = df.copy()

            for index, row in df.iterrows():
                if row['VCF_REF'] == 'N' or re.search(missing_ref_pattern, row['VCF_REF']):

                    if chrom_cache_name != row['#CHROM']:
                        chrom_cache_seq = ref_fa.fetch(row['#CHROM'])
                        chrom_cache_name = row['#CHROM']

                    df.loc[index, 'VCF_REF'] = chrom_cache_seq[row['VCF_POS'] - 1 : row['VCF_POS'] - 1 + len(row['VCF_REF'])].upper()
                    #df.loc[index, 'VCF_REF'] = ref_fa.fetch(row['#CHROM'], row['VCF_POS'] - 1, row['VCF_POS'] - 1 + len(row['VCF_REF'])).upper()
...
def explode_alt(df):
    ...
    vcf_alt = df['VCF_ALT'].apply(lambda val: str(val).split(','))
